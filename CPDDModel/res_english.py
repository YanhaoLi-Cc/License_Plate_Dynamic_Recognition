...
def train():
    ...
    X = Dense(mc.CLASSIFICATION_COUNT_ENU, activation="softmax", name="fc" + str(34), kernel_initializer=glorot_uniform(seed=0))(X)
    ...
    for i in range(33):
        ...
        if i == 0:
            ...
            model = tensorflow.keras.models.load_model(mc.RES_ENU_MODEL_PATH_H5)
            model.fit(normalized_data, train_labels, epochs=j, batch_size=34)
        # 保存ResNet模型为H5文件
        model.save(mc.RES_ENU_MODEL_PATH_H5)
        ...
        # 加载模型
        model2 = tensorflow.keras.models.load_model(mc.RES_ENU_MODEL_PATH_H5)
        # 进行预测
