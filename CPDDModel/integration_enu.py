...
# SVM Model
# 读取数据与标签
SVM_data, SVM_labels = ld.load_path_svm(mc.Predict_DIR_ENU, mc.LABEL_DICT_ENU)
# 标准化数据
SVM_data = nm.normalize_data(SVM_data)
# 调整数据尺寸为SVM模型对应大小
SVM_data = SVM_data.reshape(SVM_data.shape[0], mc.IMAGE_WIDTH * mc.IMAGE_HEIGHT)
# 装载模型
model_SVM = joblib.load(mc.SVM_ENU_PATH)
print("SVM模型装载完毕")
# 开始预测
...
# MLP Model
# 读取数据与标签
MLP_data, MLP_labels = ld.load_path(mc.Predict_DIR_ENU, mc.LABEL_DICT_ENU)
# 标准化数据
MLP_data = nm.normalize_data(MLP_data)
# 调整数据尺寸为MLP模型对应大小
MLP_data = MLP_data.reshape(MLP_data.shape[0], mc.IMAGE_WIDTH * mc.IMAGE_HEIGHT * 3)
# 装载模型
model_MLP = tf.keras.models.load_model(mc.MLP_ENU_MODEL_PATH_H5)
print("MLP模型装载完毕")
...
# ResNet Model
# 读取数据与标签
RES_data, RES_labels = ld.load_path(mc.Predict_DIR_ENU, mc.LABEL_DICT_ENU)
# 标准化数据
RES_data = nm.normalize_data(RES_data)
# 装载模型
model_RES = tf.keras.models.load_model(mc.RES_ENU_MODEL_PATH_H5)
print("RES模型装载完毕")
# 对输出结果进行argmax axis=1
